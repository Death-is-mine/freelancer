    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh]" onClick={() => setOpen(false)}>
      ...
      <div className="relative bg-surface-container-lowest w-full max-w-lg rounded-2xl border border-outline-variant/10 shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 border-b border-outline-variant/5">
          ...
          />
          <kbd className="text-label-sm text-on-surface-variant/40 bg-surface-container-high px-1.5 py-0.5 rounded">ESC</kbd>
        </div>
        <div className="max-h-64 overflow-y-auto p-2">
          {filtered.length === 0 && <p className="text-body-md text-on-surface-variant/70 text-center py-6">No results</p>}
          {filtered.map((item, i) => (
            <button key={item.path} onClick={() => navigate(item.path)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${i === idx ? "bg-surface-container-high text-on-surface" : "text-on-surface hover:bg-surface-container/50"}`}>
              <span className="text-label-sm text-on-surface-variant w-8">{item.path.split("/")[1]}</span>

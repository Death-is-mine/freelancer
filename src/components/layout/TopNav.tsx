...
export const TopNav = memo(function TopNav() {
  ...
  useEffect(() => {
    ...
    const goOnline = () => {
      setSyncState('syncing')
      doSync().finally(() => setSyncState('synced'))
    }
  ...
  useEffect(() => {
    ...
    try {
      const clients = JSON.parse(localStorage.getItem('fos_clients') || '[]')
      ...
      const projects = JSON.parse(localStorage.getItem('fos_projects') || '[]')
      for (const p of projects) {
        if (p.client?.toLowerCase().includes(q) || p.requirement?.toLowerCase().includes(q))
          results.push({
            label: `${p.client} — ${p.requirement}`,
            href: `/projects/${p.id}`,
            type: 'Project',
          })
      }
      const leads = JSON.parse(localStorage.getItem('fos_leads') || '[]')
      for (const l of leads) {
        if (l.name?.toLowerCase().includes(q) || l.company?.toLowerCase().includes(q))
          results.push({ label: l.name, href: '#', type: 'Lead' })
      }
    } catch {}
  ...
  return (
    <header
      className="h-16 w-full sticky top-0 z-40 bg-surface/80 backdrop-blur-md flex justify-between items-center px-margin-x border-b border-outline-variant/5"
      role="banner"
    >
      <div className="flex items-center flex-1">
        <div ref={searchRef} className="relative w-96 max-w-full">
          ...
          {searchResults.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-surface-container-lowest border border-outline-variant/10 rounded-xl shadow-2xl z-50 overflow-hidden">
              {searchResults.map((r) => (
                <Link
                  key={r.label + r.type}
                  href={r.href}
                ...
                >
                  <span className="text-label-xs text-on-surface-variant/60 uppercase w-14 shrink-0">
                    {r.type}
                  </span>
      ...
      <div className="flex items-center gap-4 relative">
        <div
          className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-surface-container-low"
          aria-label={`Sync status: ${syncLabels[syncState]}`}
        >
          <span
            className={`material-symbols-outlined text-[18px] ${syncColors[syncState]} ${syncState === 'syncing' ? 'animate-spin' : ''}`}
            aria-hidden="true"
          >
            {syncState === 'syncing'
              ? 'sync'
          ...
          <span className={`text-[11px] font-semibold ${syncColors[syncState]}`}>
        ...
        <div ref={notifRef} className="relative">
          <button
            ...
            className="p-2 rounded-full hover:bg-surface-container-high text-on-surface-variant transition-all relative"
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 w-5 h-5 rounded-full bg-error text-[10px] font-bold text-on-error flex items-center justify-center">
          ...
          {notifOpen && (
            <div
              className="absolute top-full right-0 mt-2 w-80 bg-surface-container-lowest border border-outline-variant/10 rounded-xl shadow-2xl z-50 max-h-96 flex flex-col"
            ...
            >
              ...
              <div className="overflow-y-auto flex-1">
                {notifications.length === 0 ? (
                  <p className="text-body-md text-on-surface-variant/60 text-center py-8">
                    No notifications
                  </p>
                ) : (
                  notifications.slice(0, 20).map((n) => (
                    <div
                      key={n.key}
                      className={`flex items-start gap-3 px-4 py-3 border-b border-outline-variant/5 hover:bg-surface-container-high transition-colors ${!n.read ? 'bg-primary/[0.02]' : ''}`}
                    >
                      <div
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-body-md ${n.read ? 'text-on-surface-variant/80' : 'text-on-surface font-medium'}`}
                        >
                          {n.msg}
                        </p>
                      ...
                      <button
                        ...
                        className="shrink-0 text-on-surface-variant/40 hover:text-error transition-colors"
              ...
              <button
                ...
                className="w-full py-2.5 text-label-md font-semibold text-primary hover:bg-primary/5 transition-colors border-t border-outline-variant/10 cursor-pointer"
              >
        ...
        <div ref={menuRef} className="relative">
          ...
          >
            ...
            {avatarUrl ? (
              <img
                src={avatarUrl}
                alt=""
                className="w-11 h-11 rounded-full border-2 border-surface-container-high object-cover"
              />
            ) : (
              <div
                className="w-11 h-11 rounded-full bg-secondary-container border-2 border-surface-container-high flex items-center justify-center text-on-secondary-container font-bold text-sm"
                aria-hidden="true"
              >
                {initials}
              </div>
          ...
          {profileOpen && (
            <div
              className="absolute top-full right-0 mt-2 w-56 bg-surface-container-lowest border border-outline-variant/10 rounded-xl shadow-2xl py-2 z-50"
              onClick={(e) => e.stopPropagation()}
            >

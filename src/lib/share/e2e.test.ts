...
describe("Share E2E workflow (JSON repo)", () => {
  ...
  it("full lifecycle: create → resolve → revoke → expired", async () => {
    const share = await repo.create({
      ...
      projectSnapshot: { client: "E2E Client", requirement: "E2E Project", amount: "10000", amountStatus: "Active", dueDate: "2026-09-01", invoiceNum: "INV-001", agreementNum: "AGR-001" },
      expiresAt: null,
      enabled: true,
      ownerId: "e2e-user",
    })
  ...
  it("owner can list their shares", async () => {
    ...
    await repo.create({ projectId: "lp1", projectSnapshot: {}, expiresAt: null, enabled: true, ownerId: "e2e-user-2" })
    await repo.create({ projectId: "lp2", projectSnapshot: {}, expiresAt: null, enabled: true, ownerId: "e2e-user-2" })
  ...
  it("cleanupExpired removes only expired", async () => {
    ...
    await repo.create({ projectId: "clean-1", projectSnapshot: {}, expiresAt: past, enabled: true, ownerId: "cleaner" })
    await repo.create({ projectId: "clean-2", projectSnapshot: {}, expiresAt: null, enabled: true, ownerId: "cleaner" })

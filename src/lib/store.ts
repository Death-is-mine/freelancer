...
async function idbPut(store: string, value: unknown) {
  try {
    const { put } = await import('./offline')
...
async function idbDelete(store: string, id: string) {
  try {
    const { del } = await import('./offline')
    await del(store, id)
  } catch {}
}

export function getClients(): Client[] {
  return load('fos_clients', [])
}

function saveClients(clients: Client[]) {
  save('fos_clients', clients)
}

function migrateClients(projects: Project[]) {
  ...
  for (const p of projects) {
    ...
    if (c && !p.clientId) {
      p.clientId = c.id
      c.revenue += Number(p.amount.replace(/[^0-9.]/g, '')) || 0
    }
...
function migrateAmounts(projects: Project[]) {
  ...
  for (const inv of invoices) {
    if (inv.amount && inv.amount.startsWith('$')) {
      inv.amount = inv.amount.replace(/^\$/, '')
      invChanged = true
    }
  }
...
export function getProjects(): Project[] {
  ...
  migrateClients(projects)
  migrateAmounts(projects)
  return projects
}

export function getLeads(): Lead[] {
  return load('fos_leads', [])
}
...
export function stopTimer(projectId: string, entryId: string) {
  ...
  addActivity(
    projectId,
    'timer',
    `Timer stopped: ${entry.description} (${Math.round(entry.duration / 60000)}m)`,
  )
}
...
export function updateLead(id: string, upd: Partial<Lead>) {
  ...
  if (idx !== -1) {
    Object.assign(leads[idx], upd)
    save('fos_leads', leads)
    idbPut('leads', leads[idx])
  }
}
...
export async function revokeShare(shareId: string) {
  try {
    await fetch(`/api/share?id=${encodeURIComponent(shareId)}`, { method: 'DELETE' })
  } catch {
    /* ignore */
  }
}
...
export function evaluateRules(trigger: RuleTrigger, context: Record<string, string>) {
  ...
  for (const rule of rules) {
    ...
    if (rule.action === 'update_status' && context.projectId) {
      ...
      if (p) {
        p.amountStatus = rule.config
        save('fos_projects', projects)
        addActivity(context.projectId, 'update', `Status auto-updated to ${rule.config}`)
      }
    }
    if (rule.action === 'send_email') {
      ...
      const email = {
        id: crypto.randomUUID().slice(0, 8),
        to: context.email || '',
        subject: rule.config.replace(/\{(\w+)\}/g, (_, k) => context[k] || ''),
        body: rule.config,
        createdAt: new Date().toISOString(),
      }
...
const DEFAULT_RULES: Rule[] = [
  {
    ...
    action: 'notify',
    config: 'New lead: {name} from {company}',
    enabled: true,
  },
  {
    id: 'r2',
    ...
    action: 'notify',
    config: 'Invoice {number} is overdue for {client}',
    enabled: true,
  },
  {
    id: 'r3',
    name: 'Auto-archive paid projects',

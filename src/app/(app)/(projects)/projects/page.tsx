...
import {
  getProjects as store,
  addProject as storeAdd,
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-body-md text-on-surface-variant mt-1">
            Track client projects, invoices, and agreements.
          </p>
        </div>
        <button
          ...
          <tbody className="divide-y divide-outline-variant/5">
            ...
            {list.map((p) => (
              <tr key={p.id} className="hover:bg-surface-container/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <select
                    value={p.amountStatus}
                    onChange={(e) => {
                      storeUpdate(p.id, { amountStatus: e.target.value })
                      setList([...store()])
                    }}
                    aria-label={`Payment status for ${p.client}`}
                    className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border-0 outline-none ${p.amountStatus === 'Paid' ? 'bg-secondary-container text-on-secondary-container' : p.amountStatus === 'Overdue' ? 'bg-error-container text-on-error-container' : 'bg-surface-container-high text-on-surface-variant'}`}
                  >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <p className="text-label-sm text-on-surface-variant/80">
                A calendar reminder will be set 1 day before the due date.
              </p>
            </div>

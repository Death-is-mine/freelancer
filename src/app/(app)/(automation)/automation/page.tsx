...
import { getRules, addRule, updateRule, deleteRule, type Rule, type RuleTrigger, type RuleAction } from "@/lib/store"
...
const ACTIONS: { value: RuleAction; label: string; hint: string }[] = [
  ...
  { value: "send_email", label: "Send Email", hint: "Queue an email to the trigger context (needs mail provider)" },
]

export default function AutomationPage() {
  const [rules, setRules] = useState<Rule[]>([])
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({ name: "", trigger: "lead.created" as RuleTrigger, action: "notify" as RuleAction, config: "" })
  ...
  function handleCreate() {
    ...
    addRule({ id: crypto.randomUUID().slice(0, 8), name: form.name.trim(), trigger: form.trigger, action: form.action, config: form.config, enabled: true })
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => setShowForm(true)} className="px-5 py-2.5 rounded-xl bg-primary text-on-primary font-semibold text-body-md shadow-lg shadow-primary/10 inline-flex items-center gap-2 cursor-pointer">
            ...
            <tbody className="divide-y divide-outline-variant/5">
              {rules.map((rule) => (
                <tr key={rule.id} className="hover:bg-surface-container/50 transition-colors">
                  <td className="px-6 py-4 text-body-md font-medium text-on-surface">{rule.name}</td>
                  <td className="px-6 py-4 text-body-md text-on-surface">{TRIGGERS.find((t) => t.value === rule.trigger)?.label || rule.trigger}</td>
                  <td className="px-6 py-4 text-body-md text-on-surface">{ACTIONS.find((a) => a.value === rule.action)?.label || rule.action}{rule.config ? `: ${rule.config}` : ""}</td>
                  <td className="px-6 py-4">
                    <button onClick={() => handleToggle(rule.id, !rule.enabled)} className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold cursor-pointer ${rule.enabled ? "bg-secondary-container text-on-secondary-container" : "bg-surface-container-high text-on-surface-variant"}`}>
                      {rule.enabled ? "Active" : "Inactive"}
                    </button>
                  </td>
                  <td className="px-6 py-4">
                    <button onClick={() => handleDelete(rule.id)} className="text-label-sm text-error cursor-pointer hover:underline" aria-label={`Delete ${rule.name}`}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative bg-surface-container-lowest w-full max-w-lg rounded-2xl border border-outline-variant/10 shadow-2xl p-6 z-10" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <input placeholder={form.action === "notify" ? 'Message (use {name}, {company}, {number}, {client})' : form.action === "create_task" ? 'Task title (use {name}, {company}, etc.)' : form.action === "send_email" ? 'Subject line (use {name}, {email}, etc.)' : 'Status value (e.g. "Paid")'} aria-label="Rule configuration" value={form.config} onChange={(e) => setForm({ ...form, config: e.target.value })} className="w-full bg-surface-container-high border-none rounded-lg px-4 py-2.5 text-body-md outline-none focus:ring-2 focus:ring-primary/20" />
              <p className="text-label-sm text-on-surface-variant/80">{ACTIONS.find((a) => a.value === form.action)?.hint}</p>
            </div>
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowForm(false)} className="px-4 py-2 rounded-xl border border-outline-variant/30 text-on-surface text-label-md cursor-pointer">Cancel</button>
              <button onClick={handleCreate} className="px-5 py-2 rounded-xl bg-primary text-on-primary text-label-md font-semibold cursor-pointer">Create Rule</button>
            </div>
          </div>
        </div>
      )}

  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/20 mt-8">
        ...
        <p className="text-body-md text-on-surface-variant mb-4">Snapshot all data to the connected Google Sheet for safekeeping.</p>
        <BackupSection />
      </div>
    </div>
  )
...
function BackupSection() {
  ...
  useEffect(() => { setLastBackup(getLastBackupTime()) }, [])
  return (
    <div>
      {msg && <div className="mb-3 px-4 py-2 rounded-lg bg-surface-container-high text-body-sm text-on-surface" role="status">{msg}</div>}
      <div className="flex items-center gap-4 flex-wrap">
        <p className="text-label-sm text-on-surface-variant">Last backup: {lastBackup ? new Date(lastBackup).toLocaleString() : "Never"}</p>
        <button onClick={async () => { setBusy(true); const ts = await backupToSheets(); if (ts) { flash(`Backup complete — ${new Date(ts).toLocaleString()}`); refresh() } else { flash("Backup failed — check SHEETS_ID and auth") }; setBusy(false) }} disabled={busy} className="px-4 py-2 rounded-lg bg-primary text-on-primary text-label-md font-semibold disabled:opacity-40 flex items-center gap-1.5">
          <span className="material-symbols-outlined text-[18px]" aria-hidden="true">{busy ? "sync" : "cloud_upload"}</span>
          {busy ? "Backing up…" : "Backup Now"}
        </button>
        <button onClick={async () => { if (!confirm("Restore will overwrite all local data. Continue?")) return; setBusy(true); const ts = await restoreFromBackup(); if (ts) { flash(`Restored from backup — ${new Date(ts).toLocaleString()}`); refresh() } else { flash("Restore failed") }; setBusy(false) }} disabled={busy} className="px-4 py-2 rounded-lg border border-outline-variant/30 text-on-surface text-label-md disabled:opacity-40 flex items-center gap-1.5">

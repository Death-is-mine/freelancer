    ...
    <div className="flex-1 overflow-y-auto max-w-4xl mx-auto w-full">
      <div className="mb-10">
        ...
        <p className="text-body-md text-on-surface-variant">
          Customize how FreelanceOS looks on your device.
        </p>
      </div>
          ...
          {(['Light', 'Dark', 'System'] as const).map((mode) => {
            ...
            return (
              <label key={mode} className="cursor-pointer">
                <div
                  className={`aspect-video rounded-xl border-2 transition-all overflow-hidden p-3 flex flex-col gap-2 shadow-sm ${selected ? 'bg-white border-primary' : 'bg-white border-transparent hover:border-primary'}`}
                >
      ...
      <div className="mb-12 bg-white p-6 rounded-2xl shadow-sm border border-outline-variant/20">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-body-md text-on-surface-variant">
              The primary color used for buttons, links, and indicators.
            </p>
          </div>
          <div className="flex gap-2">
            {ACCENTS.map((color) => (
              <button
                ...
                className={`w-8 h-8 rounded-full border-2 transition-all ${accent === color ? 'border-primary scale-110' : 'border-white'}`}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {PRESETS.map((p) => (
            <button
              ...
              onClick={() => {
                setPreset(p.name)
                selectAccent(p.colors[0])
              }}
              className={`text-left p-4 rounded-xl border cursor-pointer ${preset === p.name ? 'border-primary bg-secondary-container/50' : 'border-outline-variant hover:bg-surface-container'}`}
      ...
      <div className="mt-12 flex items-center justify-end gap-4">
        <button
          onClick={discard}
          className="px-6 py-2.5 text-label-md text-on-surface-variant hover:bg-surface-container-high rounded-lg transition-colors"
        >
          Discard changes
        </button>
        <button
          onClick={save}
          className="px-8 py-2.5 bg-primary text-white text-label-md font-semibold rounded-lg shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
        >
          Save Settings
        </button>
      </div>

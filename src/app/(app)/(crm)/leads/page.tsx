...
import {
  getLeads as store,
  addLead as storeAdd,
  deleteLead as storeDelete,
  updateLead as storeUpdate,
...
export default function LeadsPage() {
  ...
  function findDup(e: string, c: string) {
    return store().find(
      (l) =>
        l.email.toLowerCase() === e.toLowerCase() || l.company.toLowerCase() === c.toLowerCase(),
    )
  }
  ...
  function handleConvert(leadId: string) {
    ...
    if (proj) {
      setLeads([...store()])
      evaluateRules('lead.converted', {
        name: proj.client,
        projectId: proj.id,
        company: proj.requirement,
      })
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-outline-variant/30 text-on-surface font-semibold text-body-md hover:bg-surface-container-low"
          >
            Import CSV
          </button>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {stats.map((s) => (
          <div
            key={s.label}
            className="bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/5"
          >
          ...
          <tbody className="divide-y divide-outline-variant/5">
            {leads.length === 0 && (
              <tr>
                ...
                >
                  No leads yet. Import a CSV or add manually.
                </td>
              </tr>
            )}
            {leads.map((lead) => (
              <tr key={lead.id} className="hover:bg-surface-container/50 transition-colors group">
                ...
                <td className="px-6 py-4 flex items-center gap-2 min-w-[200px]">
                  <select
                    value={lead.status}
                    onChange={(e) => {
                      storeUpdate(lead.id, { status: e.target.value })
                      setLeads([...store()])
                    }}
                  ...
                  {(lead.status === 'Lost' || lead.status === 'Disqualified') && (
                    <input
                      value={lead.reason || ''}
                      onChange={(e) => {
                        storeUpdate(lead.id, { reason: e.target.value })
                        setLeads([...store()])
                      }}
                      placeholder="Reason…"
                  ...
                  <button
                    ...
                    className="p-1 rounded hover:bg-error/10 text-on-surface-variant opacity-0 group-hover:opacity-100 transition-all"
      ...
      <div
        className={`fixed inset-0 z-[100] flex items-center justify-center ${showForm ? '' : 'hidden'}`}
      >
            ...
            <button
              onClick={() => {
                ...
                if (dup) {
                  setToast({
                    show: true,
                    msg: `Duplicate: "${dup.name}" already exists with this email/company`,
                  })
                  setTimeout(() => setToast({ show: false, msg: '' }), 3000)
                  return
                }

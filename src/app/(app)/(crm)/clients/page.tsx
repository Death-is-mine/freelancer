...
export default function ClientsPage() {
  ...
  return (
    <>
      <h2 className="text-headline-lg tracking-tight text-on-surface mb-8">Clients {clients.length > 0 && `(${clients.length})`}</h2>
      {clients.length === 0 ? (
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {c.email && <p className="text-label-sm text-on-surface-variant">{c.email}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-body-md text-on-surface">{projectCounts[c.id] || 0}</td>
                  <td className="px-6 py-4 text-body-md font-semibold text-on-surface">{formatNumber(revenues[c.id] || 0)}</td>
                  <td className="px-6 py-4">
                    <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-secondary-container text-on-secondary-container">{c.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>

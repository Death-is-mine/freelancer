...
export default function ClientDetailPage() {
  ...
  useEffect(() => {
    ...
    setClientProjects(
      found
        ? allProjects.filter(
            (p) =>
              (p.clientId && p.clientId === found.id) ||
  ...
  const totalAmount = clientProjects.reduce(
    (s, p) => s + (Number(p.amount.replace(/[^0-9.]/g, '')) || 0),
    0,
  )
  const paidAmount = clientProjects
    .filter((p) => p.amountStatus === 'Paid')
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
        ...
        <p className="text-body-md text-on-surface-variant">
          &ldquo;{clientName}&rdquo; does not have any projects.
        </p>
        <button
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-surface-container-lowest p-5 rounded-2xl border border-outline-variant/5">
          ...
          <h3 className="text-headline-md font-bold mt-1 text-secondary">
        ...
        <div className="bg-surface-container-lowest p-5 rounded-2xl border border-outline-variant/5">
          ...
          <span className="inline-block mt-2 px-3 py-1 rounded-full text-[11px] font-bold bg-secondary-container text-on-secondary-container">
          ...
          <tbody className="divide-y divide-outline-variant/5">
            {clientProjects.map((p) => (
              <tr key={p.id} className="hover:bg-surface-container/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold ${p.amountStatus === 'Paid' ? 'bg-secondary-container text-on-secondary-container' : p.amountStatus === 'Overdue' ? 'bg-error-container text-on-error-container' : 'bg-surface-container-high text-on-surface-variant'}`}
                  >
                    {p.amountStatus}
                  </span>
                </td>

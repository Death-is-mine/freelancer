...
const defaultMembers: Member[] = [
  { id: "1", name: "You", email: "you@example.com", role: "Owner", status: "Active", joined: "Jan 2026" },
]
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => setShowInvite(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-on-primary font-semibold text-body-md shadow-lg shadow-primary/10">
          <span className="material-symbols-outlined text-[20px]" aria-hidden="true">person_add</span>Invite Member
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        {[
          ...
          { label: "Active", value: members.filter((m) => m.status === "Active").length, icon: "check_circle", color: "text-success" },
          { label: "Pending Invites", value: pendingInvites.length, icon: "hourglass_empty", color: "text-warning" },
        ].map((s) => (
        ...
        <table className="w-full">
          <thead>
            <tr className="text-left border-b border-outline-variant/10">
              <th className="px-6 py-4 text-label-sm text-on-surface-variant uppercase">Member</th>
              <th className="px-6 py-4 text-label-sm text-on-surface-variant uppercase">Role</th>
              <th className="px-6 py-4 text-label-sm text-on-surface-variant uppercase">Status</th>
              <th className="px-6 py-4 text-label-sm text-on-surface-variant uppercase">Joined</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-outline-variant/5">
            {members.map((m) => (
              <tr key={m.id} className="hover:bg-surface-container/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <select value={m.role} onChange={(e) => setMembers(members.map((x) => x.id === m.id ? { ...x, role: e.target.value as Member["role"] } : x))} aria-label={`Role for ${m.name}`} className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border-0 outline-none ${m.role === "Owner" ? "bg-primary/10 text-primary" : "bg-surface-container-high text-on-surface-variant"}`}>
                    {roles.map((r) => <option key={r} value={r}>{r}</option>)}
                  </select>
                </td>
                <td className="px-6 py-4">
                  <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold ${m.status === "Active" ? "bg-secondary-container text-on-secondary-container" : "bg-warning/10 text-warning"}`}>{m.status}</span>
                </td>
      ...
      <div className="bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={r.role} className="p-4 rounded-xl bg-surface-container-low border border-outline-variant/5">
        ...
        <p className="text-label-sm text-on-surface-variant/60 mt-6">Integrates with Google Workspace for SSO and directory sync.</p>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative bg-surface-container-lowest w-full max-w-md rounded-2xl border border-outline-variant/10 shadow-2xl p-6 z-10" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <input aria-label="Email address" placeholder="colleague@company.com" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} className="w-full bg-surface-container-high border-none rounded-lg px-4 py-2.5 text-body-md outline-none focus:ring-2 focus:ring-primary/20" />
              <select aria-label="Role" value={inviteRole} onChange={(e) => setInviteRole(e.target.value as Member["role"])} className="w-full bg-surface-container-high border-none rounded-lg px-4 py-2.5 text-body-md outline-none focus:ring-2 focus:ring-primary/20">
                {roles.filter((r) => r !== "Owner").map((r) => <option key={r} value={r}>{r}</option>)}
              </select>
            </div>
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowInvite(false)} className="px-4 py-2 rounded-xl border border-outline-variant/30 text-on-surface text-label-md">Cancel</button>
              <button onClick={handleInvite} className="px-5 py-2 rounded-xl bg-primary text-on-primary text-label-md font-semibold">Send Invite</button>
            </div>
          </div>
        </div>
      )}

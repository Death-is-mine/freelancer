...
function buildReports(): ReportData {
  ...
  return {
    'Revenue Report': {
      icon: 'bar_chart',
      data: months.map((month, i) => ({ month, value: revByMonth[i] })).filter((r) => r.value > 0),
    },
    'Client Report': {
      icon: 'group',
      data: months
        .map((month, i) => ({ month, value: clientsByMonth[i] }))
        .filter((r) => r.value > 0),
    },
    'Project Report': {
      icon: 'work',
      data: months.map((month, i) => ({ month, value: projByMonth[i] })).filter((r) => r.value > 0),
    },
      ...
      {selected ? (
        <div>
          <button
            ...
            className="flex items-center gap-2 text-label-md text-primary font-semibold mb-6 hover:underline"
          >
            ...
            {reportData[selected]?.data.length === 0 ? (
              <p className="text-body-md text-on-surface-variant">
                {selected === 'Lead Sources'
                  ? 'Import leads to see conversion by source.'
                  : 'No data yet. Create projects to see reports.'}
              </p>
            ) : selected === 'Lead Sources' ? (
                ...
                {(() => {
                  ...
                  return sorted.map(([source, counts], i) => (
                    <div key={i} className="flex items-center gap-4">
                      <span
                        className="text-label-md text-on-surface-variant w-28 truncate"
                        title={source}
                      >
                        {source}
                      </span>
                      ...
                      <span className="text-label-md font-semibold text-on-surface w-12 text-right">
                        {counts.total}
                      </span>
                      <span className="text-label-md text-secondary w-16 text-right">
                        {counts.converted}/{counts.total}
                      </span>
                      <span className="text-label-md font-semibold text-on-surface w-14 text-right">
                        {counts.total > 0
                  ...
                  <div key={i} className="flex items-center gap-4">
                    ...
                    <div className="flex-1 h-6 bg-surface-container-high rounded-full overflow-hidden">
                      <div
                        className="h-full bg-primary rounded-full transition-all"
                        style={{
                          width: `${(d.value / maxValue(reportData[selected].data)) * 100}%`,
                        }}
                      ></div>
                    </div>
                    <span className="text-label-md font-semibold text-on-surface w-20 text-right">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {Object.entries(reportData).map(([title, r], i) => (
            <button
              ...
              className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant/5 shadow-sm hover:shadow-md transition-shadow cursor-pointer text-left"
            >

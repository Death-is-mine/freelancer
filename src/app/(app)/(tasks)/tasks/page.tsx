...
function saveTasks(tasks: Task[]) {
  try {
    localStorage.setItem('fos_tasks', JSON.stringify(tasks))
  } catch {}
}
...
const defaults: Task[] = [
  {
    id: '1',
    title: 'Finalize Lumina branding assets',
    priority: 'High',
    project: 'Lumina',
    date: 'Today',
    done: false,
  ...
  {
    id: '2',
    title: 'Send Q4 invoice to Stellar Media',
    priority: 'Medium',
    project: 'Apex Systems',
    date: 'Today',
    done: false,
  },
  {
    id: '3',
    title: 'Review Nexus Systems proposal',
    priority: 'Urgent',
    project: 'Nexus',
    date: 'Today',
    done: false,
        ...
        <div className="mb-6 bg-surface-container-lowest rounded-2xl border border-outline-variant/20 p-4">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <button
                type="submit"
                className="px-4 py-2 rounded-lg bg-primary text-on-primary text-label-md font-semibold"
              >
                Add Task
              </button>
              <button
      ...
      <div className="bg-surface-container-lowest rounded-3xl border border-outline-variant/10 shadow-sm overflow-hidden">
        {visible.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <span
              className="material-symbols-outlined text-4xl text-on-surface-variant/30 mb-3"
              aria-hidden="true"
            >
              check_circle
            </span>
            <p className="text-body-md text-on-surface-variant">
              {filter === 'done' ? 'No completed tasks yet' : 'All tasks are done!'}
            </p>
          </div>
        ) : (
          visible.map((task) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={`text-body-md ${task.done ? 'line-through text-on-surface-variant/70' : 'text-on-surface'}`}
                >
                  {task.title}
                </p>
              ...
              <div
                className={`w-2 h-2 rounded-full shrink-0 ${priorityColors[task.priority] || 'bg-secondary'}`}
              ></div>
            </div>
          ))
        )}

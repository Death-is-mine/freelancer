...
import {
  getProjects as projectStore,
  updateProject,
  generateInvoice as storeGenerateInvoice,
} from '@/lib/store'
...
export default function PaymentsPage() {
  ...
  function generateInvoice() {
    ...
    if (pending.length === 0) {
      showToast('All projects already have invoices.')
      return
    }
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-body-md text-on-surface-variant mt-1">
            Generate invoices and track payments against projects.
          </p>
        </div>
        <button
            ...
            {withInvoices.map((proj) => (
              <tr key={proj.invoiceNum} className="hover:bg-surface-container/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <select
                    value={proj.amountStatus}
                    onChange={(e) => {
                      updateProject(proj.id, { amountStatus: e.target.value })
                      refresh()
                    }}
                    aria-label={`Invoice status for ${proj.client}`}
                    className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border-0 outline-none ${proj.amountStatus === 'Paid' ? 'bg-secondary-container text-on-secondary-container' : proj.amountStatus === 'Overdue' ? 'bg-error-container text-on-error-container' : 'bg-surface-container-high text-on-surface-variant'}`}
                  >

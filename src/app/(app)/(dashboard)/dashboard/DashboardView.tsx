    ...
    <>
      <div className="mb-8 flex items-start justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => router.push("/leads")} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-surface-container-high text-on-surface text-body-md font-semibold hover:bg-surface-container-highest transition-colors">
            <span className="material-symbols-outlined text-[18px]" aria-hidden="true">leaderboard</span>
            New Lead
          </button>
          <button onClick={() => router.push("/projects?new=true")} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-on-primary font-semibold text-body-md shadow-lg shadow-primary/10">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-1 bg-surface-container-high rounded-lg p-0.5">
            {(["daily", "weekly", "monthly", "yearly"] as GraphPeriod[]).map((p) => (
              <button key={p} onClick={() => setGraphPeriod(p)} className={`px-3 py-1.5 rounded-md text-label-sm capitalize ${graphPeriod === p ? "bg-surface-container-lowest text-on-surface font-semibold shadow-sm" : "text-on-surface-variant hover:text-on-surface"}`}>{p}</button>
            ))}
          </div>
        </div>
          ...
          {Object.entries(graphData).map(([label, value]) => (
            <div key={label} className="text-center">
              <div className="h-32 flex items-end justify-center mb-2">
                <div className={`w-8 rounded-t-lg transition-all ${graphColors[label]}`} style={{ height: `${(value / graphMax) * 100}%` }}></div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant/5 shadow-sm">
          <div className="flex items-center gap-3 mb-1">
            <span className="material-symbols-outlined text-warning" aria-hidden="true">assignment_late</span>
            <span className="text-label-md text-on-surface-variant uppercase tracking-wider">Overdue Projects</span>
          </div>
          <p className="text-headline-md font-bold text-on-surface mt-2">{overdueCount}</p>
          <p className="text-body-md text-on-surface-variant mt-1">{overdueCount > 0 ? "Require attention" : "All on track"}</p>
        </div>
        <div className="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant/5 shadow-sm">
          <div className="flex items-center gap-3 mb-1">
          ...
          <p className="text-body-md text-on-surface-variant mt-1">{pendingAgreements > 0 ? `${pendingAgreements} need signatures` : "All set"}</p>
        </div>
      </div>

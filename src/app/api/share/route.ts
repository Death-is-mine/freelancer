...
export async function POST(request: Request) {
  if (!rateLimit('share-create', request, 20, 60000))
  ...
  if (!projectId || !projectSnapshot)
    return NextResponse.json({ error: 'Missing projectId or projectSnapshot' }, { status: 400 })
...
export async function GET(request: Request) {
  if (!rateLimit('share-list', request, 60, 60000))
    return NextResponse.json({ error: 'Too many requests' }, { status: 429 })
  const ownerId = await getServerOwnerId()
  if (!ownerId) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 })
...
export async function DELETE(request: Request) {
  if (!rateLimit('share-revoke', request, 30, 60000))
    return NextResponse.json({ error: 'Too many requests' }, { status: 429 })
  const ownerId = await getServerOwnerId()
  if (!ownerId) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 })

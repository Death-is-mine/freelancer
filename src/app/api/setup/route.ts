...
export async function POST() {
  ...
  for (const name of subfolders) {
    const existing = await driveRequest(`/files?q=name%3D%27${encodeURIComponent(name)}%27+and+%27${rootId}%27+in+parents+and+trashed%3Dfalse`, accessToken)
  ...
  for (const name of sheetNames) {
    await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${name}!A1:Z1`,
      {
        method: "PUT",
        headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
        body: JSON.stringify({ range: `${name}!A1`, values: [headers[name]], majorDimension: "ROWS" }),
      },
    )
  }

...
async function ensureSheet(accessToken: string) {
  ...
  const has = data.sheets?.some((s: { properties: { title: string } }) => s.properties.title === SHEET_NAME)
  if (has) return
  ...
  await fetch(`${SHEETS_BASE}/${sid}/values/${RANGE}?valueInputOption=USER_ENTERED`, {
    method: "PUT",
    headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
    body: JSON.stringify({ range: RANGE, values: [["timestamp", "data"], [new Date().toISOString(), ""]], majorDimension: "ROWS" }),
  })
}

export async function POST(request: Request) {
  if (!rateLimit("backup-write", request, 10, 60000)) return NextResponse.json({ error: "Too many requests" }, { status: 429 })
  ...
  try {
    ...
    const res = await fetch(`${SHEETS_BASE}/${sid}/values/${RANGE}:append?valueInputOption=USER_ENTERED`, {
      method: "POST",
